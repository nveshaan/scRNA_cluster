...
from algorithms.seurat import seurat_like_clustering, seurat_scvi_clustering, seurat_linear_scvi_clustering
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.dataset == "pbmc":
        ...
        adata.obs['ground_truth'] = adata.obs['cell_type']  # Assuming 'leiden' column has the original clusters
    else:
        print("Loading simulated dataset...")
        adata = sc.datasets.blobs(n_cells=3000, n_genes=2000, centers=5, cluster_std=0.5, random_state=42)
        adata.obs['ground_truth'] = adata.obs['leiden']  # 'leiden' is the default cluster label for blobs
    ...
    if cfg.method == "pca":
        ...
        save_embedding_only(adata, basis='X_umap_pca', label_key='seurat_leiden', run_dir=run_dir, cfg=cfg, ari=ari)
    elif cfg.method == "scvi":
        print("Running scVI...")
        adata, model = seurat_scvi_clustering(adata, cfg)
        ari = adjusted_rand_score(adata.obs['seurat_scvi_leiden'], adata.obs['ground_truth'])
        save_embedding_only(adata, basis='X_umap_scvi', label_key='seurat_scvi_leiden', run_dir=run_dir, cfg=cfg, ari=ari)
        ...
        ari = adjusted_rand_score(adata.obs['seurat_linear_scvi_leiden'], adata.obs['ground_truth'])
        save_embedding_only(adata, basis='X_umap_linear_scvi', label_key='seurat_linear_scvi_leiden', run_dir=run_dir, cfg=cfg, ari=ari)
        model.save(run_dir / "checkpoints")
    else:
